...
// https://blog.csdn.net/liyuanbhu/article/details/11119081
...
//			return null;
//
//		LysBoardPoint[] outs = new LysBoardPoint[ins.size()];
//
//		double[] in = new double[ins.size() * 2];
//		double[] out = new double[ins.size() * 2];
//
//		for (int i = 0; i < ins.size(); i++)
//		{
//			LysBoardPoint p = ins.get(i);
//			in[i * 2] = p.x;
//			in[i * 2 + 1] = p.y;
//		}
//
//		linearSmooth3(in, out, ins.size() * 2);
//
//		for (int i = 0; i < out.length; i += 2)
//		{
//			LysBoardPoint p = new LysBoardPoint(out[i], out[i + 1]);
//			outs[i / 2] = p;
//		}
//
//		return outs;
//	}
	...
	public static LysBoardPoint[] smooth(List<LysBoardPoint> ins)
	{
		...
		double[] inx = new double[ins.size()];
		double[] iny = new double[ins.size()];
		double[] outx = new double[ins.size()];
		double[] outy = new double[ins.size()];
		...
		cubicSmooth7(inx, outx, ins.size());
		cubicSmooth7(iny, outy, ins.size());

		for (int i = 0; i < ins.size(); i++)
		{
			LysBoardPoint p = new LysBoardPoint((float) outx[i], (float) outy[i]);
			outs[i] = p;
		}
	...
	public static void quadraticSmooth5(double in[], double out[], int N)
	{
		int i;
		if (N < 5)
		{
	...
	public static void quadraticSmooth7(double in[], double out[], int N)
	{
		int i;
		if (N < 7)
		{
		...
		{
			out[0] = (32.0 * in[0] + 15.0 * in[1] + 3.0 * in[2] - 4.0 * in[3] - 6.0 * in[4] - 3.0 * in[5] + 5.0 * in[6]) / 42.0;

			out[1] = (5.0 * in[0] + 4.0 * in[1] + 3.0 * in[2] + 2.0 * in[3] + in[4] - in[6]) / 14.0;

			out[2] = (1.0 * in[0] + 3.0 * in[1] + 4.0 * in[2] + 4.0 * in[3] + 3.0 * in[4] + 1.0 * in[5] - 2.0 * in[6]) / 14.0;
			for (i = 3; i <= N - 4; i++)
			{
				out[i] = (-2.0 * (in[i - 3] + in[i + 3]) + 3.0 * (in[i - 2] + in[i + 2]) + 6.0 * (in[i - 1] + in[i + 1]) + 7.0 * in[i]) / 21.0;
			}
			out[N - 3] = (1.0 * in[N - 1] + 3.0 * in[N - 2] + 4.0 * in[N - 3] + 4.0 * in[N - 4] + 3.0 * in[N - 5] + 1.0 * in[N - 6] - 2.0 * in[N - 7]) / 14.0;
	...
	public static void cubicSmooth5(double in[], double out[], int N)
	{
	...
	public static void cubicSmooth7(double in[], double out[], int N)
	{
		int i;
		if (N < 7)
		{
